...
import be.kdg.airene.ports.in.SaveNotificationPort;
...
@AllArgsConstructor
@Slf4j
public class NotificationAdapter implements SaveNotificationPort, LoadNotificationsLastTwoWeeksByUserIdPort {
	...
	public List<Notification> loadNotificationsLastTwoWeeksByUserId(UUID userId, String sort) {
		Sort sortBy = sort.equalsIgnoreCase(Sort.Direction.ASC.name()) ? Sort.by("timestamp").ascending() : Sort.by("timestamp").descending();
